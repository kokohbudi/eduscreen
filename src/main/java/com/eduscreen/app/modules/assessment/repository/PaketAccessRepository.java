...
public interface PaketAccessRepository extends JpaRepository<PaketAccessEntity, UUID> {
    ...
    /**
     * Akses yang boleh dipakai untuk pemakaian baru: belum dicabut, belum lewat batas, dan
     * Paketnya masih terbit — Paket yang ditarik Eduscreen berhenti dipakai baru (AC-B10, FR-068).
     */
    @Query("select a from PaketAccessEntity a, PaketEntity p where a.paketId = p.id "
            + "and p.publishedAt is not null and a.clientId = :clientId and a.revokedAt is null "
    ...
    @Query("select a from PaketAccessEntity a, PaketEntity p where a.paketId = p.id "
            + "and p.publishedAt is not null and a.clientId = :clientId and a.paketId = :paketId "
